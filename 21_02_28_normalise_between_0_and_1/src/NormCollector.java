...
public class NormCollector implements Collector<Double, ArrayList<Double>, Double> {
    ...
    public Function<ArrayList<Double>, Double> finisher() {
        return doubleStream -> { // our lambda function to calculate the result
            ...
            // doubleStream.map(element -> ((element - dataLow) / (dataHigh - dataLow)) * (normalizedHigh - normalizedLow) + normalizedLow);

            return ((doubleStream
                    .stream()
                    .map(doubleValue -> (doubleValue - dataLow) / (dataHigh - dataLow)) * (normalizedHigh - normalizedLow) + normalizedLow));
